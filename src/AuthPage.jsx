    ...
    return (
      <div style={{ background: 'linear-gradient(135deg, #E6F7F5, #F3F3EE)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Noto Sans', sans-serif" }}>
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, maxWidth: 400, width: '100%', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#0D9488', marginBottom: 8, textAlign: 'center' }}>🏪 ShopEasy POS</h1>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #e0e0e0', fontSize: 14, paddingRight: '40px' }}
                placeholder="Enter your password"
              />
              <button
                onClick={() => setShowPassword(!showPassword)}
                style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 16 }}
              >
                {showPassword ? '👁️' : '👁️‍🗨️'}
              </button>
            </div>
          ...
          <button
            onClick={handleLogin}
            disabled={loading}
            style={{ width: '100%', padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#0D9488', color: '#fff', border: 'none', cursor: 'pointer', marginBottom: 12, opacity: loading ? 0.6 : 1 }}
          >
            {loading ? 'Logging in...' : 'Login'}
          </button>

          <button
            onClick={() => { setStep('register'); setError(''); setPhoneNumber(''); setPassword(''); }}
            style={{ width: '100%', padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#f0f0f0', color: '#0D9488', border: 'none', cursor: 'pointer' }}
          >
    ...
    return (
      <div style={{ background: 'linear-gradient(135deg, #E6F7F5, #F3F3EE)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Noto Sans', sans-serif" }}>
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, maxWidth: 400, width: '100%' }}>
          ...
          <button
            onClick={() => setStep('selectIndustry')}
            style={{ width: '100%', padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#0D9488', color: '#fff', border: 'none', cursor: 'pointer', marginBottom: 12 }}
          >
          ...
          <button
            ...
            style={{ width: '100%', padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#f0f0f0', color: '#666', border: 'none', cursor: 'pointer' }}
          >
            Back to Login
          </button>
        </div>
        ...
        <div style={{ maxWidth: 900, width: '100%' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 20 }}>
            {Object.entries(INDUSTRIES).map(([key, ind]) => (
              <div
                ...
                style={{
                  background: selectedIndustry === key ? '#E6F7F5' : '#fff',
                  borderRadius: 16,
                  padding: 20,
                  cursor: 'pointer',
                  border: selectedIndustry === key ? '2px solid #0D9488' : '2px solid #eee',
                  transition: 'all 0.2s',
                  textAlign: 'center',
                }}
              >
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button
              ...
              style={{ flex: 1, padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#f0f0f0', color: '#666', border: 'none', cursor: 'pointer' }}
            >
              Back
            </button>
            <button
              ...
              disabled={loading}
              style={{ flex: 1, padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#0D9488', color: '#fff', border: 'none', cursor: 'pointer', opacity: loading ? 0.6 : 1 }}
            >
